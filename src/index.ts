...
const parsePagesDirectory = (
  ...
  let files = siblings
    .filter(
      (f) => f.isFile() && f.name.endsWith('.tsx') && !f.name.startsWith('-'),
    )
    .map((f) => f.name);
  ...
  if (files.includes('_layout.tsx')) {
    ...
    routeOptions.push(
      `"element": "() => import('/${path.join(dir, '_layout.tsx')}')"`,
    );
    ...
    if (files.includes('404.tsx')) {
      const route404Options = [];
      route404Options.push(`"name": "NotFound"`);
      route404Options.push(
        `"element": "() => import('/${path.join(dir, '404.tsx')}')"`,
      );
  ...
  for (const name of remainingDirectories) {
    const parsedDir = parsePagesDirectory(path.join(dir, name), {
      ...
      prependPath: `${prependPath}${parseDynamicRoutes(name).replace(
        /^_/,
        ':',
      )}/`,
    });
...
// 插件函数 返回值是一个插件对象
const React_Auto_Routes = (
  { pagesDir } = { pagesDir: 'src/pages/' },
): Plugin => {
